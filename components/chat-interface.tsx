...
export function ChatInterface({ conversationId }: ChatInterfaceProps = {}) {
  ...
  const { initializeWindow, loadConversationsFromDB, setActiveChat, activeChat, chats, getCurrentChat, lastPromotedChatId } = useChatStore()
  ...
  useEffect(() => {
    ...
    // Only route to the conversation page when the active chat transitions from temp -> permanent
    if (!conversationId && currentChat && !currentChat.isTemporary && currentChat.permanentId && lastPromotedChatId === currentChat.permanentId) {
  ...
  }, [conversationId, getCurrentChat, router, chats, lastPromotedChatId])
  ...
  return (
    <div className="relative flex h-screen w-screen flex-row" style={{backgroundColor: 'var(--token-main-surface-primary)'}}>
      {/* Sidebar Rail (desktop) + Sheet (mobile) */}
      <div className="hidden md:block relative z-21 h-full shrink-0 overflow-hidden transition-all duration-300" aria-label="Sidebar">
        <div className={`${sidebarCollapsed ? 'w-16' : 'w-64'} h-full`}
             role="complementary">
          <ChatSidebar 
      ...
      <div className="relative flex h-full max-w-full flex-1 flex-col" role="main" id="main-content" aria-label="Main content">
        ...
        {/* Desktop Header */}
        <header className="draggable sticky top-0 p-2 items-center justify-between z-20 h-14 pointer-events-none select-none hidden md:flex" style={{backgroundColor: 'var(--token-main-surface-primary)'}} role="banner" aria-label="Application header">
